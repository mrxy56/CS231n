...
class CaptioningRNN(object):
  ...
  def loss(self, features, captions):
    ...
    if self.cell_type == 'rnn':
        out, cache = rnn_forward(word_embedding_out, affine_out, Wx, Wh, b)
    elif self.cell_type == 'lstm':
        out, cache = lstm_forward(word_embedding_out, affine_out, Wx, Wh, b)
    else:
        raise ValueError('Invalid cell_type "%s"' % self.cell_type)
    temporal_affine_out, temporal_affine_cache = temporal_affine_forward(out, W_vocab, b_vocab)
    loss, dtemporal_affine_out = temporal_softmax_loss(temporal_affine_out, captions_out, mask)
    dout, grads['W_vocab'], grads['b_vocab'] = temporal_affine_backward(dtemporal_affine_out, temporal_affine_cache)
    if self.cell_type == 'rnn':
        dword_embadding_out, daffine_out, grads['Wx'], grads['Wh'], grads['b'] = rnn_backward(dout, cache)
    elif self.cell_type == 'lstm':
        dword_embadding_out, daffine_out, grads['Wx'], grads['Wh'], grads['b'] = lstm_backward(dout, cache)
    else:
        raise ValueError('Invalid cell_type "%s"' % self.cell_type)
    grads['W_embed'] = word_embedding_backward(dword_embadding_out, word_embedding_cache)
  ...
  def sample(self, features, max_length=30):
    ...
    for i in range(1, max_length):
        ...
        captions[:,i] = np.argmax(vocab_affine_out, axis = 1)
